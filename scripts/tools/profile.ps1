...
if ($ProfileStartTick -lt 0 -or $ProfileStartTick -ge $Ticks) {
    throw "ProfileStartTick must be greater than or equal to zero and smaller than Ticks."
}
...
$arguments = @(
    ...
    "--profile",
    "--ticks", $Ticks,
    "--profile-start-tick", $ProfileStartTick
)
...
if ($exitCode -ne 0) {
    throw "Fixed-tick performance profile failed."
}

Write-Host ("Process wall-clock time: {0:N3} s" -f $stopwatch.Elapsed.TotalSeconds)
